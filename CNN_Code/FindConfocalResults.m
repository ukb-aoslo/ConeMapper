...
% GTDPDir = fullfile(BasePath,'Images and Results','Confocal','ValGTDPcoord_BonnAll'); % for all man-CNN comparisons
% GTDPDir = fullfile(BasePath,'Images and Results','Confocal','Locs_AllGrader1_txtWMH'); % for comparisons between manual graders
GTDPDir = fullfile(BasePath,'Images and Results','Confocal','Locs_AllGrader1_txtWMH'); % for comparisons with 712 pixel images
...
% scaleInfoFile = fullfile(BasePath,'Images and
% Results','Confocal','scale_info_Confocal.csv'); % moved selection of file to get_parameters_Cone_CNN (JLR - 20.02.2019)

scaleInfoFile = params.Results.Scale; % load the selection of file made in get_parameters_Cone_CNN (JLR - 20.02.2019)
...
%     CNNPos = conelocs_cutout;    % can be used for only comparing manual conelocs of the two individual graders 
...
FalseNegCNN = NumMissesCNN./(NumMatchesCNN+NumMissesCNN);
FileDiceCNN = (2*NumMatchesCNN)./(2*NumMatchesCNN +NumFalseAlarmsCNN+NumMissesCNN);

disp('Sensitivity   FDR       FN        Dice')
disp('--------------------------------------')
  
MeanResultsCNN = [mean(FileSensCNN) mean(FileFDRCNN) mean(FalseNegCNN) mean(FileDiceCNN)]
StdResultsCNN = [std(FileSensCNN) std(FileFDRCNN) std(FalseNegCNN) std(FileDiceCNN)]


FileSensGTDP = NumMatchesGTDP./(NumMatchesGTDP+NumMissesGTDP);
FileFDRGTDP = NumFalseAlarmsGTDP./(NumMatchesGTDP+NumFalseAlarmsGTDP);
FalseNegGTDP = NumMissesGTDP./(NumMatchesGTDP+NumMissesGTDP);
FileDiceGTDP = (2*NumMatchesGTDP)./(2*NumMatchesGTDP +NumFalseAlarmsGTDP+NumMissesGTDP);
  
MeanResultsGTDP = [mean(FileSensGTDP) mean(FileFDRGTDP) mean(FalseNegGTDP) mean(FileDiceGTDP)]
StdResultsGTDP = [std(FileSensGTDP) std(FileFDRGTDP) std(FalseNegGTDP) std(FileDiceGTDP)]
...
X = linspace(60000,180000,400);         % for plotting foveal cone-densities JLR (2019-21-02)
